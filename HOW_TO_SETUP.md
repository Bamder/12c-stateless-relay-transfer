...
```
浏览器 (Client Web，由 Registry :8080 分发)
    │  reserve / register / 下载
    ▼
...
| 组件 | 默认地址 | 作用 |
|------|----------|------|
| Registry + Client | `http://127.0.0.1:8080` | 元数据与路由；**同时托管 Client Web**（`dist/` 已构建时） |
| Relay | `http://127.0.0.1:9090` | 无状态块存储；由 Registry 分配 |
| Console | `http://127.0.0.1:8070` | 运维 UI + BFF 代理 |
| Client Web（开发） | `http://127.0.0.1:5173` | Vite 热更新调试（改前端时用） |
...
#### 方式 A：Registry 托管（推荐，免 Vite、适合穿透）

Client 已并入 Registry。首次需构建生产包：

```powershell
cd client

# 首次：安装 emsdk 并构建（约需数分钟～数十分钟）
.\build.ps1 -SetupEmsdk -EmsdkRoot P:\_Tools\emsdk -Production

# 之后仅更新前端时
.\build.ps1 -Production
```

构建完成后**重启 Registry**，浏览器打开 **http://127.0.0.1:8080**。  
Registry 会动态下发同源 `relay.config.json`，无需手填 Registry URL。

> SakuraFrp 等穿透场景：只需给 Registry **1 条隧道**；Relay 另需 1 条。穿透 HTTPS 时建议 Registry 前置反代并传递 `X-Forwarded-Proto` / `X-Forwarded-Host`。

#### Cloudflare Quick Tunnel（无需账号，适合扫码联调）

仓库未内置 SakuraFrp 客户端或凭据。若没有现成的 SakuraFrp 隧道，可在 Registry、Relay 和 Console 已启动且 Relay 已获批后运行：

```powershell
cd server
.\start-cloudflare-public.ps1
```

脚本会启动两条 HTTPS Quick Tunnel（Registry 与 Relay 各一条），自动把 Relay 公网地址同步到本地配置和 Registry Allowlist，并重启 Relay。随后用脚本输出的 `Registry / QR URL` 打开客户端，生成的二维码即可由手机访问。

Quick Tunnel 域名会在进程重启后变化，旧二维码也会随之失效，只建议用于测试。停止隧道：

```powershell
.\stop-cloudflare-public.ps1
```

长期部署应使用 Cloudflare Named Tunnel 或已有 SakuraFrp 固定域名，并保持 Registry 与 Relay 的 HTTPS 公网地址稳定。

#### 方式 B：Vite 开发服务器

适合改前端代码、热更新调试。详见 [Client 构建](#client-构建)：
...
也可通过 npm 调用：`npm run build`、`npm run start`（在 `client/` 目录）。此时 `relay.config.json` 仍指向 `http://127.0.0.1:8080`。
...
**由 Registry 托管 Client 时**（`http://127.0.0.1:8080`），Registry 动态返回同源 `/relay.config.json`，一般无需配置。

**Vite 开发模式**（`:5173`）读取 `client/web/public/relay.config.json`：
